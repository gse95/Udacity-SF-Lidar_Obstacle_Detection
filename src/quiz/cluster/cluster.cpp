...
void proximityCluster(const std::vector<std::vector<float>>& points, int id, KdTree* tree, float distanceTol, std::vector<bool> &f_p, std::vector<int> &cluster)
{
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	std::vector<int> nearby = tree->search({7,6.4},3.0);
